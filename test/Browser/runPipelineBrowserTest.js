...
test('runPipelineBrowser executes pre and post calls to run embind calls, etc', (t) => {
  const pipelinePath = 'Embind'
  const args = ['input.txt', 'input.bin', 'output.txt', 'output.bin']
  const desiredOutputs = [
    { path: 'output.txt', type: IOTypes.Text }
  ]
  ...
  return runPipelineBrowser(pipelinePath, args, desiredOutputs, inputs, preFunction, postFunction)
    .then(function ({stdout, stderr, outputs}) {
      t.is(outputs[0].path, 'output.txt')
      t.is(outputs[0].type, IOTypes.Text)
      t.is(outputs[0].data, 'The answer is 42.')
